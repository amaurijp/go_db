...
- `manually_extracted_parameters.json` contains manually annotated parameters from 40 articles on graphene oxide (GO), classified into eight application-based topics: 1, Catalysis; 2, Energy Storage; 3, Polymer and Composite Reinforcement; 4, Soft Matter and Scaffolds; 5, Biomedical and Drug Delivery; 6, Water Treatment and Membranes; 7, Nanosensors; and 8, Films and Coatings. This ground-truth (GT) dataset can be used as a benchmark for information-extraction tasks performed by LLMs.
- `extraction_validation_data.json`: contains the extraction validation set (VD) used to calculate precision, recall and F1-score of the LLM extraction task performed in the article submitted to ACS Nano (nn-2026-04548e).
...
Each article-level entry in `extraction_validation_data.json` contains two main sections:
